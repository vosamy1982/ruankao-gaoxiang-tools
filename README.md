...
- [复习中心](https://vosamy1982.github.io/ruankao-gaoxiang-tools/#reviewCenter)
...
- 考点收藏、学习状态、进度统计和状态筛选
- 独立复习中心，支持智能队列、继续学习、前后切换和随机抽取
- 今日学习计划，支持每日目标、逐项完成、进度恢复和按日重置
- 学习记录本地持久化，支持独立导入、导出和清空
- 浏览器内导入、校验、编辑和导出私有考点 JSON
- 下拉式顶部导航，按 ITTO 查询、学习复习、资料速查和内容维护组织模块
- 开发模式下保存源码数据与粘贴图片上传
...
2. 在顶部导航选择对应下拉菜单，再进入具体视图；移动端点击 `菜单` 后按分组进入：
   - `ITTO 查询`：交叉矩阵、Excel 检索、反向搜索。
   - `学习复习`：考点阅览、复习中心、知识辨析。
   - `资料速查`：项目文件、绩效域。
   - `内容维护`：后台维护。
...
学习记录保存在当前浏览器的 `localStorage` 中，与考点内容 JSON 分开。`考点精炼阅览` 和 `复习中心` 共用同一份学习记录；刷新和重新打开页面后会自动恢复。清理浏览器站点数据前，建议先导出学习记录备份。

## 快速复习

快速复习已经独立到 `复习中心`，不再占用 `考点精炼阅览` 的目录空间。在复习中心选择队列，然后点击 `继续学习`：

`今日计划` 默认从智能复习队列中安排 5 个考点，也可以调整为 10、15 或 20 个。点击 `完成本项` 后会记录今日进度并自动进入下一个未完成考点；刷新页面后可以继续。日期变化时系统会保留目标数量并生成新一天的计划。今日完成只表示当天复习过该考点，不会自动修改它的学习状态。
...
进入队列后，可以使用 `上一个`、`随机` 和 `下一个` 导航，并查看当前考点在本轮队列中的位置。复习队列是本次会话的快照，修改学习状态不会中断当前轮次；重新点击 `继续学习` 会按当前学习记录重新生成队列。
...
当前公开版已具备本地学习记录、独立复习中心、今日学习计划、基础数据审计和自动化测试。后续维护重点是继续提高原创示例质量、完善间隔复习能力和控制前端包体积。
